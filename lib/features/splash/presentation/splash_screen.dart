...
//     final stripePaint = Paint()..color = const Color(0xFFFFB300);
...
//       ..color = const Color(0xFFB0BEC5).withOpacity(0.6)
//       ..strokeWidth = 2.5
...
//       final paint = Paint()
//         ..color = const Color(0xFF1565C0).withOpacity(p.opacity * 0.2);
//       canvas.drawCircle(
      ...
      body: Stack(
        children: [
          ...
          const Positioned(
            ...
            left: 0,
            right: 0,
            child: _DecorativeCircles(isTop: true),
          ),
          ...
          const Positioned(
            ...
            left: 0,
            right: 0,
            child: _DecorativeCircles(isTop: false),
          ),
...
// ── Decorative Circles (top & bottom) ────────────────────────────────────────
class _CircleData {
  final double xFraction; // 0.0–1.0 of screen width
  final double size;
...
class _DecorativeCircles extends StatelessWidget {
  final bool isTop;
  const _DecorativeCircles({required this.isTop});

  static const List<_CircleData> _circles = [
    _CircleData(xFraction: 0.04, size: 90, opacity: 0.10),
    _CircleData(xFraction: 0.22, size: 50, opacity: 0.07),
    _CircleData(xFraction: 0.45, size: 36, opacity: 0.06),
    _CircleData(xFraction: 0.62, size: 56, opacity: 0.08),
    _CircleData(xFraction: 0.80, size: 44, opacity: 0.07),
    _CircleData(xFraction: 0.95, size: 110, opacity: 0.10),
  ];
        ...
        return SizedBox(
          ...
          child: Stack(
            ...
            children: _circles.map((c) {
              final left = c.xFraction * width - c.size / 2;
              // Top circles hang off the top edge; bottom circles hang off bottom
              final top = isTop ? -c.size * 0.45 : -c.size * 0.35;
              return Positioned(
                left: left,
                top: top,
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    color: const Color(0xFF1565C0).withValues(alpha: c.opacity),
                  ),
                ),
              );
            }).toList(),
...
class _TrainPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final windowPaint = Paint()..color = Colors.white.withValues(alpha: 0.9);
    final wheelPaint = Paint()..color = const Color(0xFF37474F);
    final glowPaint = Paint()
    ...
    // Yellow accent stripe along the bottom of body
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(0, size.height - 26, size.width, 5),
        const Radius.circular(2),
    ...
    // Headlight glow
    final headlightPaint = Paint()
      ..color = const Color(0xFFFFFFAA).withValues(alpha: 0.9)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8);
    canvas.drawCircle(
...
class _TrackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 20; i++) {
      final x = i * (size.width / 18);
      canvas.drawLine(Offset(x, 6), Offset(x, 30), tiePaint);
    }
  }
...
class _ParticlePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (final p in particles) {
      final y = (p.y - progress * p.speed * 20) % 1.0;
      final paint = Paint()
        ..color = const Color(0xFF1565C0).withValues(alpha: p.opacity * 0.2);
      canvas.drawCircle(
        Offset(p.x * size.width, y * size.height),
        p.size,
        paint,
