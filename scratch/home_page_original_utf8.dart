...
import 'package:rafiq_metrro/core/theme/theme_cubit.dart';
import 'package:rafiq_metrro/core/utils/gamification_service.dart';
import 'package:rafiq_metrro/core/utils/metro_data.dart';
import 'package:rafiq_metrro/core/utils/responsive.dart';
import 'package:rafiq_metrro/core/utils/voice_service.dart';
import 'package:rafiq_metrro/features/ai_assistant/presentation/pages/ai_assistant_page.dart';
import 'package:rafiq_metrro/features/community/presentation/pages/community_page.dart';
import 'package:rafiq_metrro/features/community/presentation/pages/lost_and_found_page.dart';
import 'package:rafiq_metrro/features/metro/domain/entities/station.dart';
import 'package:rafiq_metrro/features/metro/presentation/cubits/nearby_stations_cubit.dart';
import 'package:rafiq_metrro/features/metro/presentation/pages/ar_navigation_page.dart';
import 'package:rafiq_metrro/features/metro/presentation/pages/line_alerts_page.dart';
import 'package:rafiq_metrro/features/metro/presentation/pages/map_page.dart';
import 'package:rafiq_metrro/features/metro/presentation/pages/nearby_stations_page.dart';
import 'package:rafiq_metrro/features/metro/presentation/pages/route_planner_page.dart';
import 'package:rafiq_metrro/features/metro/presentation/pages/subscription_optimizer_page.dart';
import 'package:rafiq_metrro/features/metro/presentation/widgets/feature_card.dart';
import 'package:rafiq_metrro/features/metro/presentation/widgets/tourist_translator_modal.dart';
import 'package:rafiq_metrro/features/news/presentation/pages/news_page.dart';
import 'package:rafiq_metrro/features/pricing_calculator/presentation/pages/pricing_calculator_page.dart';
import 'package:rafiq_metrro/features/tourism/presentation/pages/tourist_attractions_page.dart';
import 'package:rafiq_metrro/features/trip_scheduler/presentation/pages/trip_scheduler_page.dart';
import 'package:rafiq_metrro/features/voice_command/presentation/voice_command_service.dart';
...
class _HomePageState extends State<_HomePageView> {
  // ΓöÇΓöÇ Quick trip planner ΓÇô uses ALL stations (user picks line automatically) ΓöÇ
  String? _fromStation;
  String? _toStation;

  // ΓöÇΓöÇ Metro type toggle (for info section only) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        actions: [
          ...
          PopupMenuButton<Locale>(
            ...
            itemBuilder: (context) => [
              ...
              const PopupMenuItem(value: Locale('ar'), child: Text('╪º┘ä╪╣╪▒╪¿┘è╪⌐')),
              const PopupMenuItem(value: Locale('fr'), child: Text('Fran├ºais')),
          ...
          child: ConstrainedBox(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  // ΓöÇΓöÇ Greeting ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                  FadeInDown(
                    child: Text(
                      "Where to today? ≡ƒÜç".tr(),
                  ...
                  FadeInDown(
                    ...
                    child: Text(
                      "Enter your start & destination ΓÇô we'll find the way".tr(),
                      style: const TextStyle(color: AppColors.textSecondary, fontSize: 14),
                    ),
                  ),
                  SizedBox(height: r.sectionSpacing),

                  // ΓöÇΓöÇ BIG TRIP PLANNER card (all stations) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                  FadeInDown(
                  ...
                  // ΓöÇΓöÇ Metro type info toggle ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                  FadeInDown(
                  ...
                  // ΓöÇΓöÇ Nearest Station (GPS-based, real-time) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                  FadeInUp(
                    delay: const Duration(milliseconds: 140),
                  ...
                  // ΓöÇΓöÇ Map shortcut ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                  FadeInUp(
                  ...
                  // ΓöÇΓöÇ Feature cards ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Nearest Station Live (GPS-based) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("≡ƒôì Nearest Metro Station".tr(), style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                        ...
                        Row(
                          children: [
                            ...
                            Text(isAr ? '╪º┘ä╪«╪╖ ${s.line}' : 'Line ${s.line}', style: TextStyle(color: lineColor, fontSize: 11, fontWeight: FontWeight.w600)),
  ...
  // ΓöÇΓöÇ Main Trip Planner (all stations, no line filter) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSearchableStationDropdown(
              context: context,
              isAr: isAr,
              label: "≡ƒôì From (Start Station)".tr(),
              value: _fromStation,
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: () {
                  ...
                  if (_fromStation == _toStation) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text("ΓÜá∩╕Å Choose a different destination!".tr()),
  ...
  // ΓöÇΓöÇ Metro Type Toggle (┘à╪¬╪▒┘ê ╪º┘ä┘é╪º┘ç╪▒╪⌐ / ┘é╪╖╪▒ ╪º┘ä╪╣╪º╪╡┘à╪⌐) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    final types = [
      {
        ...
        'color': AppColors.primary,
        'sub': "3 Lines ΓÇó 85 Stations".tr(),
      },
      {
  ...
  // ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  // ΓöÇΓöÇ Capital Metro info card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _capitalInfoRow(Icons.route_outlined, "Monorail: Adly Mansour Γåö Admin. Capital".tr(), isAr),
  ...
  // ΓöÇΓöÇ Map shortcut ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    isAr ? '≡ƒù║∩╕Å ╪«╪▒┘è╪╖╪⌐ ╪«╪╖┘ê╪╖ ╪º┘ä┘à╪¬╪▒┘ê' : '≡ƒù║∩╕Å Metro Lines Map',
                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15),
                  ),
                  Text(
                    isAr ? '╪┤┘ê┘ü ┘â┘ä ╪º┘ä╪«╪╖┘ê╪╖ ┘ê╪º┘ä┘à╪¡╪╖╪º╪¬' : 'View all lines & stations',
  ...
  // ΓöÇΓöÇ SingleΓÇæcolumn list (phone portrait) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ 2ΓÇæcolumn grid (tablet / landscape) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  List<Widget> _featureConfigs(BuildContext context) {
    ...
    return [
      ...
      // ΓöÇΓöÇ NEW FEATURE 1: AI Trip Assistant ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      FeatureCard(
        title: "AI Assistant ≡ƒñû".tr(),
      ...
      // ΓöÇΓöÇ Feature 3: Trip Scheduler ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      FeatureCard(
        title: "Trip Scheduler ≡ƒôà".tr(),
      ...
      // ΓöÇΓöÇ NEW FEATURE 5: Pricing Calculator ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      FeatureCard(
        title: "Cost Calculator ≡ƒÆ│".tr(),
      ...
      // Tourist Attractions
      FeatureCard(
        title: "Tourist Attractions ≡ƒù║∩╕Å".tr(),
        subtitle: "Discover Egypt landmarks from any station ΓÇó 4 languages".tr(),
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(isAr ? '╪º┘ä╪«╪╖ $lineNum' : 'Line $lineNum', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                    ...
                    SnackBar(
                      ...
                      content: Row(
                        children: [
                          ...
                          const SizedBox(width: 12),
                          Text(isAr ? '╪¬┘à ╪¬┘ü╪╣┘è┘ä ╪º┘ä╪¬┘å╪¿┘è┘ç╪º╪¬ ┘ä┘ä╪«╪╖ $lineNum' : 'Alerts enabled for Line $lineNum'),
                        ],
                      ),
  ...
  Widget _buildStatusCard(String line, String status, Color color, BuildContext context, Responsive r) {
    final isOnTime = status == 'on_time'.tr() || status == '┘à╪│╪¬┘é╪▒';
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      lang == 'ar' ? '$stationCount ┘à╪¡╪╖╪⌐' : '$stationCount stations',
...
// ΓöÇΓöÇ Fallback Voice Command Sheet ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
class _VoiceCommandSheet extends StatelessWidget {
  ...
  void _navigate(BuildContext context, String cmd) {
    ...
    Widget? dest;
    if (lower.contains('╪«╪▒┘è╪╖╪⌐') || lower.contains('map')) {
      dest = const MapPage();
    } else if (lower.contains('┘à╪«╪╖╪╖') || lower.contains('route') || lower.contains('╪▒╪¡┘ä╪⌐') || lower.contains('plan')) {
      dest = const RoutePlannerPage();
    } else if (lower.contains('┘é╪▒┘è╪¿') || lower.contains('nearby')) {
      dest = const NearbyStationsPage();
    } else if (lower.contains('┘à╪¼╪¬┘à╪╣') || lower.contains('community')) {
      dest = const CommunityPage();
    } else if (lower.contains('╪ú╪«╪¿╪º╪▒') || lower.contains('news')) {
      dest = const NewsPage();

    } else if (lower.contains('╪¼╪»┘ê┘ä') || lower.contains('schedule')) {
      dest = const TripSchedulerPage();
    } else if (lower.contains('╪¬┘â┘ä┘ü╪⌐') || lower.contains('price') || lower.contains('╪¡╪º╪│╪¿╪⌐') || lower.contains('calculator')) {
      dest = const PricingCalculatorPage();
    } else if (lower.contains('╪░┘â╪º╪í') || lower.contains('ai') || lower.contains('┘à╪│╪º╪╣╪»') || lower.contains('assistant')) {
  ...
  Widget build(BuildContext context) {
    final quickCmds = isAr
        ? ['╪º┘ä╪«╪▒┘è╪╖╪⌐', '┘à╪«╪╖╪╖ ╪º┘ä╪▒╪¡┘ä╪⌐', '╪º┘ä╪╖┘ê╪º╪▒╪ª', '╪º┘ä╪░┘â╪º╪í ╪º┘ä╪º╪╡╪╖┘å╪º╪╣┘è']
